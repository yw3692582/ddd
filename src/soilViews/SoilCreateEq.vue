<template>
	<div>
		<div>
			<el-page-header @back="$router.push('/soilequiment')" content="新增设备"></el-page-header>
			...
			<el-card class="login-card">
				<el-form @submit.native.prevent status-icon :model="verification" ref="verification">
					...
					<el-form-item label="设备经纬度,例:113.790921,34.748171" prop="lon_lat" :rules="verification.rules">
						<el-input v-model="verification.lon_lat" onSubmit="return false" onkeyup="value=value.replace(/[^\d.,，]/g,'');" @blur.prevent="b()" placeholder="请输入经纬度"
		...
		<div class="crea_pre">
			<!-- 标定参数弹窗 -->
			<el-dialog :close-on-click-modal="false" width="90%" title="标定参数" :visible.sync="dialogParameterVisible">
				...
				<el-form :model="model" ref="model">
					<el-table :data="model.tableData" tooltip-effect="dark" border stripe style="width: 100%">
						<el-table-column label="层数" type="index" align="center"></el-table-column>
						<el-table-column label="层深" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.depth'" :rules="model.rules">
						...
						<el-table-column label="空气频率" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.air'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.air"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="水频率" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.water'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.water"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="传感器标定参数A" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.sensorA'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.sensorA"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="传感器标定参数B" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.sensorB'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.sensorB"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="传感器标定参数C" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.sensorC'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.sensorC"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="采集器标定参数C" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.collectorC'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.collectorC"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="采集器标定参数D" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.collectorD'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.collectorD"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="田间持水量" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.field'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.field"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="土壤容重" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.soil'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.soil"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="凋萎湿度" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.humidity'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.humidity"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
						<el-table-column label="土壤质地" align="center">
							<template slot-scope="scope">
								<el-form-item :prop="'tableData.' + scope.$index + '.texture'" :rules="model.rules">
									<el-input oninput="value=value.replace(/[^\d^\.]/g,'')" style="width: 80px;" v-model="scope.row.texture"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
				...
				<div slot="footer" class="dialog-footer">
					...
					<el-button type="primary" @click="editParameter('model')">确 定</el-button>
				</div>
			</el-dialog>
		</div>
	</div>
			...
			register(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$ajax.post('/ch_add_equipments', {
								e_code: '0102',
								basic_info: {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						for (let i = 0; i < this.model.tableData.length; i++) {
							this.depth.push(this.model.tableData[i].depth)
							this.air.push(this.model.tableData[i].air)
							this.water.push(this.model.tableData[i].water)
							this.sensorA.push(this.model.tableData[i].sensorA)
							this.sensorB.push(this.model.tableData[i].sensorB)
							this.sensorC.push(this.model.tableData[i].sensorC)
							this.collectorC.push(this.model.tableData[i].collectorC)
							this.collectorD.push(this.model.tableData[i].collectorD)
							this.field.push(this.model.tableData[i].field)
							this.soil.push(this.model.tableData[i].soil)
							this.humidity.push(this.model.tableData[i].humidity)
							this.texture.push(this.model.tableData[i].texture)
						}
						this.dialogParameterVisible = false
					} else {
						this.$message({
